...
def ngramcount():
    with open('bodytagwordcount400000to450000.json') as file:
        ...
        for key in keys:
            ...
            for word in words:
                cnt[word] += 1
            counts[key] = cnt
        with open('bodytag400000to450000.json', 'w') as write:
    ...
    with open('bodytagsover100.json', 'r') as file:
        ...
        for dicts in wordcounts:
            for word, count in dicts.items():
                cnt[word] += count
        with open('globalwordcountover100.json', 'w') as write:
            ujson.dump(cnt, write)


def combine_dicts():
    with open('bodytag400000to450000over10.json') as tag200, open('bodytagsover10.json') as tag1500:
        tag200dict = ujson.load(tag200)
        tag500dict = ujson.load(tag1500)
        newdict = dict(chain(tag200dict.items(), tag500dict.items()))
        with open('bodytagsover10.json', 'w') as write:
